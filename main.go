package main

import (
	"fmt"
	"lcatania/aoc-2024-go/day1"
	"lcatania/aoc-2024-go/day10"
	"lcatania/aoc-2024-go/day11"
	"lcatania/aoc-2024-go/day12"
	"lcatania/aoc-2024-go/day2"
	"lcatania/aoc-2024-go/day3"
	"lcatania/aoc-2024-go/day4"
	"lcatania/aoc-2024-go/day5"
	"lcatania/aoc-2024-go/day6"
	"lcatania/aoc-2024-go/day7"
	"lcatania/aoc-2024-go/day8"
	"lcatania/aoc-2024-go/day9"
)

func main() {
	fmt.Println("Day1 Result -", day1.Day1())
	fmt.Println("Day1 Part 2 Result -", day1.Day1Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day2 Result -", day2.Day2())
	fmt.Println("Day2 Part2 Result -", day2.Day2Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day3 Result -", day3.Day3())
	fmt.Println("Day3 Part2 Result -", day3.Day3Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day4 Result -", day4.Day4())
	fmt.Println("Day4 Part2 Result -", day4.Day4Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day5 Result -", day5.Day5())
	fmt.Println("Day5 Part2 Result -", day5.Day5Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day6 Result -", day6.Day6())
	fmt.Println("Day6 Part2 Result -", day6.Day6Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day7 Result -", day7.Day7())
	fmt.Println("Day7 Part2 Result -", day7.Day7Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day8 Result -", day8.Day8())
	fmt.Println("Day8 Part2 Result -", day8.Day8Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day9 Result -", day9.Day9())
	fmt.Println("Day9 Part2 Result -", day9.Day9Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day10 Result -", day10.Day10())
	fmt.Println("Day10 Part2 Result -", day10.Day10Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day11 Result -", day11.Day11())
	fmt.Println("Day11 Part2 Result -", day11.Day11Part2())
	fmt.Println("-----------------------------")
	fmt.Println("Day12 Result -", day12.Day12())
	fmt.Println("Day12 Part2 Result -", day12.Day12Part2())
}
